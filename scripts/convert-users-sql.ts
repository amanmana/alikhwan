...
function main() {
  ...
  if (!fs.existsSync(USERS_SQL_PATH)) {
    console.error(
      `Ralat: Fail 'users.sql' tidak ditemui di: ${USERS_SQL_PATH}`,
    );
    process.exit(1);
  }
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(OUTPUT_DIR, "import-errors.csv"),
      errors.join("\n"),
    );
    fs.writeFileSync(
      path.join(OUTPUT_DIR, "import-duplicates.csv"),
      duplicates.join("\n"),
    );
